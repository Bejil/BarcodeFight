...
public class BF_Ruby: NSObject {
		...
		timer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
			...
			BF_User.current?.update { error in
				...
				if error == nil {
					...
					BF_Toast_Manager.shared.addToast(title: String(key: "user.freeRuby.toast.title"), subtitle: String(key: "user.freeRuby.toast.subtitle"), style: .Success)
				}
				else {
	...
	private func nextDate(from date: Date) -> Date {
		...
		let calendar = Calendar.current
		guard let nextNoon = calendar.date(bySettingHour: 12, minute: 0, second: 0, of: date) else { return date }
